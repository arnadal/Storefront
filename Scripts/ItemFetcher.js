...
function RenderItems(Data = []) {
	if (Data.length == 0) {
		if (window.location.pathname == `/Purchase.html`) {
			document.querySelector(
				`div#StoreItems`
			).innerHTML = `<p>Apologies! We could not find any item that you described. Try describing in short, and avoid being too specific.</p>`;
		} else if (window.location.pathname == `/Cart.html`) {
			document.querySelector(
				`div#StoreItems`
			).innerHTML = `<p>Your cart is empty. You might want to <a href="Purchase.html">purchase some products</a>!</p>`;
		}
    ...
    NewElement.id=`finalWork`;
		NewElement.className = `ItemCard`;
		NewElement.innerHTML = `<div class="ItemImage" style="background-image: url('${
			Data[Index].Image
		}');" alt="${Data[Index].ItemName}"></div><b class="ItemName">${
			Data[Index].ItemName
		}</b><span class="ItemPrice">${Data[Index].Price}</span><br>${
			window.location.pathname.includes(`/Purchase.html`)
				? `<button id="AddToCartButton" onclick="AddToCart('${Data[Index].ItemName}')" style="position: relative; height: 3em; width: 8em"><span class="material-symbols-outlined" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); color: white">
    add_shopping_cart
    </span></button>`
				: ``
		}`;
...
async function MainCode() {
		...
		if (window.location.search.includes(`?Search=`)) {
			function FetchSearch() {
				...
				for (let Index = 0; Index < StoreItems.length; Index++) {
					if (
						StoreItems[Index].ItemName.match(new RegExp(`${Search}`, `gi`)) !=
						null
					) {
	...
	if (window.location.pathname.includes(`/Cart.html`)) {
		...
		if (Cart.length == 0) {
			document
				.querySelector(`#CheckoutStartButton`)
				.setAttribute(`disabled`, `true`);
			document
				.querySelector(`#CheckoutStartButtonDisabledText`)
